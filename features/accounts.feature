...
Feature: Creating an account
  ...
  Scenario: Anonymous user can start creating an account
    ...
    Then  she should be at the 'users/new' page
     ...
     And  she should see a <form> containing a textfield: "Email Address", password: Password, password: 'Confirm Password', submit: 'Sign up'
  ...
  Scenario: Anonymous user can not create an account with incomplete or incorrect input
    Given an anonymous user
     And  no user with login: 'Oona' exists
    When  she registers an account with login: '',     password: 'monkey', password_confirmation: 'monkey'
    Then  she should be at the 'users/new' page
     And  she should     see an errorExplanation message 'Your Email Address can't be blank'
